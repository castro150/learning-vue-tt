<template>
<div>
  <button v-if="$route.path !== '/time-entries/log-time'" class="btn btn-primary" @click="redirectToLogTime()">
    Log Time
  </button>
  ...
  data() {
    ...
    let existingEntry = {
      user: {
        ...
        image: 'http://1.gravatar.com/avatar/7f4ec37467f2f7db6fffc7b4d2cc8dc2?s=250'
      },
  ...
  methods: {
    ...
    deleteTimeEntry(timeEntry) {
      ...
      if (window.confirm('Are you sure you want to delete this time entry?')) {
        ...
        Event.$emit('delete-time', timeEntry)
      }
    }
  },
  created() {
